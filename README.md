...
A Discord bot that brings AI-powered coding assistance directly into your server. It connects to Claude Code CLI, allowing team members to submit coding tasks and collaborate on projects—all through simple slash commands.

![CodeTogether Bot in Action](codetogether.png)

## Overview

Users type commands like `/task prompt:"Fix the bug in auth.py"` and the bot sends the request to Claude. With a registered project, Claude can read files, write code, edit existing files, run commands, search the web, and perform any action the Claude Code CLI supports—working directly in your codebase with full context.
...
The system consists of a **Rust-based Discord bot** (using Serenity) that handles commands and a **Python wrapper service** that executes Claude Code CLI. This separation allows the bot to run anywhere while the wrapper runs on machines with Claude CLI access. User identities are verified server-side through Discord's API.
